...
The runner compiles the grounding, memory, telemetry, prompt, and sanitization paths into a temporary executable with small test-only Unity/BepInEx stubs. It does not start Erenshor, contact Ollama, install a DLL, or retain test files. A non-zero exit code means at least one named regression failed.
